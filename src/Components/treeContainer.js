...
export default class TreeContainer extends React.PureComponent {
  ...
  buildSubTree(root) {
    ...
    if (
      ...
      root.name.toLowerCase().indexOf(this.props.filter.toLowerCase()) !== -1
    ) {
      return root;
    }
    return null;
  ...
  setClassName(node) {
    ...
    node.className =
      node.name.toLowerCase().indexOf(this.props.filter) === -1
        ? "node searchExcluded"
        : "node searchIncluded";
  }

  render() {
    let root = this.props.activeNode
      ? this.getRoot(this.props.data)
      : this.props.data;
    ...
    return (
      <Tree
        ...
        gProps={{
          className: "node",
          onClick: (event, nodeKey) => this.props.onclick(event, nodeKey),
          onMouseOver: (event, nodeKey) =>
            this.props.handleMouseHover(event, nodeKey),
          onMouseLeave: (event, nodeKey) =>
            this.props.handleMouseHover(event, nodeKey),
        }}
        textProps={{
          dy: 3.5,
